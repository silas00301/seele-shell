...
fn parse_duration(value: &str) -> Result<u64, &'static str> {
    ...
    while index < bytes.len() {
        ...
        let count: u64 = value[start..index].parse().map_err(|_| "invalid-duration")?;
        while bytes.get(index) == Some(&b' ') {
            index += 1;
        }
        let unit = index;
...
fn project(snapshot: &Value) -> Value {
    ...
    let headline = if active { mode_label(&mode) } else { "Caffeinate" };
    let hover = if !active {
        "Caffeinate".to_owned()
...
mod tests {
    ...
    fn typed_durations_agree_on_one_hour_and_a_half() {
        for text in ["90", "90m", "90 minutes", "1h30", "1h 30m", "1:30", " 1H30M "] {
    ...
    fn unbounded_or_unreadable_durations_never_start_a_session() {
        for text in ["", "   ", "h", "1x", "1:5", "1:60", "1:305", "1h2m3m4m", "99999999999999"] {
