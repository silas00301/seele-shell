    ...
    Ok(match function {
        ...
        "loupe" => {
            ...
            let point = |value: Option<&Value>, size: u64| {
                (finite(value, 0.0).max(0.0) * size as f64)
                    .floor()
                    .min((size - 1) as f64) as u64
            };
...
mod tests {
    ...
    fn formats_follow_what_the_colour_actually_offers() {
        ...
        assert_eq!(
            call("cycle", &[exact.clone(), json!("hex")]).unwrap(),
            "rgb"
        );
        assert_eq!(
            call("cycle", &[exact.clone(), json!("rgb")]).unwrap(),
            "token"
        );
        assert_eq!(
            call("cycle", &[exact.clone(), json!("token")]).unwrap(),
            "hex"
        );
        assert_eq!(
            call("cycle", &[plain.clone(), json!("rgb")]).unwrap(),
            "hex"
        );
        assert_eq!(
            call("cycle", &[plain.clone(), json!("token")]).unwrap(),
            "hex"
        );
    ...
    fn the_lens_keeps_its_size_against_every_edge() {
        ...
        for (x, y) in [(0.0, 0.0), (0.5, 0.5), (1.0, 1.0), (-1.0, 2.0)] {
            let lens = call(
                "loupe",
                &[json!(x), json!(y), json!(1920), json!(1080), json!(9)],
            )
            .unwrap();
        ...
        let small = call(
            "loupe",
            &[json!(0.5), json!(0.5), json!(4), json!(3), json!(9)],
        )
        .unwrap();
        ...
        assert!(
            call(
                "loupe",
                &[json!(0.5), json!(0.5), json!(0), json!(0), json!(9)]
            )
            .is_err()
        );
    }
