    ...
    Ok(match function {
        ...
        "devices" => {
            ...
            let message = match lost {
                ...
                "output" => {
                    "The test output is no longer available. Choose another and start again."
                }
                _ => "",
            };
...
mod tests {
    ...
    fn the_test_output_defaults_to_the_system_output_and_keeps_an_explicit_choice() {
        ...
        let chosen = call(
            "devices",
            &[devices(), json!("sink.headphones"), idle.clone()],
        )
        .unwrap();
    ...
    fn a_device_that_disappears_under_a_running_test_ends_it_instead_of_rerouting() {
        ...
        let intact = call(
            "devices",
            &[devices(), json!("sink.headphones"), live.clone()],
        )
        .unwrap();
    ...
    fn a_test_without_a_resolved_device_reports_which_one_is_missing() {
        ...
        let no_output = call("start", &[json!("live"), json!("source.webcam"), json!("")]).unwrap();
    ...
    fn the_card_offers_only_what_the_current_state_allows() {
        ...
        let ready = view(
            json!({"mode":"idle","sample":true,"clipped":true}),
            json!({}),
        );
    ...
    fn mute_and_failure_are_reported_instead_of_being_worked_around() {
        ...
        assert_eq!(
            view(json!({"mode":"idle","muted":true}), json!({}))["muted"],
            false
        );
        let failed = view(
