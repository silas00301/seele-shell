...
fn failure(code: &str) -> &'static str {
    match code {
        ...
        "changed" => "The listener changed while the confirmation was open. Nothing was stopped; review it again.",
        "not-authorized" => "Authentication was cancelled or refused. Nothing was stopped.",
        "not-escalatable" => "Try a graceful stop first. Force stop is offered only if it does not work.",
...
mod tests {
    ...
    fn a_proposed_url_is_valid_for_both_families_or_is_not_proposed() {
        ...
        assert_eq!(url("192.168.1.10", 8443, "https"), "https://192.168.1.10:8443");
        assert_eq!(
            url("[::1]", 9000, "http"),
            "http://[::1]:9000",
    ...
    fn a_row_describes_its_own_target() {
        let service = json!({"user":"root","owners":[{}],"target":{"kind":"service","unit":"nginx.service"}});
        assert_eq!(
