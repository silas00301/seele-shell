...
or closed. Every page uses `all=true` and `participating=false`; there is no
repository, reason, type or AI-priority filter. Each page becomes visible before
...
remote images and markup are not loaded by the detail view.
...
opens the internal detail view. Delivery is limited to sixteen concurrent waiters;
...
Up/Down or J/K selects an inbox row, Enter opens its detail, O opens its exact
GitHub subject, D marks Done, R refreshes/retries, and Escape returns to the inbox.
Tab reaches the visible action buttons. Opening a row/detail makes no read-state
write. Done is optimistic, uses the documented DELETE thread endpoint, and returns
...
the complete API-returned collection minus locally confirmed Done revisions.
It cannot promise parity for state/types GitHub does not expose, nor infer remote
saved/Done state from the unread flag. GitHub's retention/access limits apply.
...
and `focus` events. QML owns Qt process lifecycle, focus and retained ListModels;
Rust owns scheduling, state, comparison and presentation blocks. EOF/signals cancel
...
navigation, detail actions and narrow/error states. These checks are wired into
