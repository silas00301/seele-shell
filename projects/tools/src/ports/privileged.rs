...
pub fn stop_service(
    ...
    let resolved = owners(roots, inode);
    if resolved.is_empty() || resolved.iter().any(|owner| owner.service != unit) {
        return Err("changed");
    }
...
///
/// The start time is what makes this safe: a recycled PID has a different one,
/// so a stale confirmation can never redirect the signal onto whatever took the
/// number over.
...
pub fn stop_process(
    ...
    if !system.signal(pid, if force { libc::SIGKILL } else { libc::SIGTERM }) {
        return Err("failed");
    }
...
impl System for Host {
    ...
    fn signal(&mut self, pid: u32, number: i32) -> bool {
        // SAFETY: kill takes no memory arguments. The PID was revalidated
        // against its start time immediately above this call.
        unsafe { libc::kill(pid as libc::pid_t, number) == 0 }
    }
}
...
mod tests {
    ...
    fn rejects_every_argument_shape_that_is_not_a_typed_target() {
        ...
        for arguments in [
            ...
            vec!["service", "reboot", "nginx.service", "1", "0.0.0.0:80", "80"],
            vec!["service", "graceful", "nginx.socket", "1", "0.0.0.0:80", "80"],
            vec![
                "service",
                "graceful",
                "user@1000.service",
            ...
            vec!["service", "graceful", "nginx.service", "01", "0.0.0.0:80", "80"],
            vec!["service", "graceful", "nginx.service", "-1", "0.0.0.0:80", "80"],
            vec![
                "service",
                "graceful",
                "nginx.service",
            ...
            vec!["service", "graceful", "nginx.service", "1", "0.0.0.0:80", "0"],
            vec![
                "service",
                "graceful",
                "nginx.service",
            ...
            vec!["process", "graceful", "1", "1", "0", "1", "0.0.0.0:80", "80"],
            vec!["shutdown", "now"],
        ] {
    ...
    fn strict_decimal_and_binding_syntax() {
        ...
        for bad in ["", "01", "+1", " 1", "1 ", "1e3", "-1", "18446744073709551616"] {
