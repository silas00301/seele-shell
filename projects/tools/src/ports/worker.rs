    ...
    fn plan(&mut self, id: &str, mode: &str) -> Value {
        ...
        let siblings: Vec<String> = self
            ...
            .filter(|other| match target.kind {
                "service" => other.service().is_some_and(|(name, scope)| {
                    name == unit && scope == target.scope
                }),
        ...
        if !proxy.is_empty() {
            disclosure.push(
                "This is a host-side proxy, not the application it forwards to.".to_owned(),
            );
        }
...
impl Host for Desktop {
    ...
    fn signal(&mut self, pid: u32, number: i32) -> bool {
        // SAFETY: kill takes no memory arguments, and the PID was revalidated
        // against its start time immediately before this call.
        pid >= 2 && unsafe { libc::kill(pid as libc::pid_t, number) == 0 }
    }
...
mod tests {
    ...
    impl Host for Recorder {
        fn systemctl(&mut self, user: bool, arguments: &[&str]) -> bool {
            self.units
                .push(format!("{}{}", if user { "--user " } else { "" }, arguments.join(" ")));
            self.succeeds
        }
    ...
    fn force_is_refused_until_a_graceful_stop_has_failed() {
        ...
        let replies = worker.request(
            &json!({"op": "stop", "token": token, "mode": "force"}).to_string(),
        );
    ...
    fn a_malformed_or_vanished_target_is_refused_before_anything_runs() {
        ...
        for token in [
            ...
            let replies =
                worker.request(&json!({"op": "stop", "token": token}).to_string());
