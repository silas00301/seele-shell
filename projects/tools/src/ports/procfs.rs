...
impl Listener {
    ...
    pub fn id(&self) -> String {
        format!("{}:{}", if self.address.is_ipv4() { 4 } else { 6 }, self.inode)
    }
        ...
        match self.address {
            ...
            IpAddr::V6(address) => {
                address.is_unspecified()
                    || address.to_ipv4_mapped().is_some_and(|v4| v4.is_unspecified())
            }
        }
    }
...
pub fn users(roots: &Roots) -> HashMap<u32, String> {
    ...
    for line in text.lines().take(8192) {
        ...
        let (Some(name), Some(_password), Some(uid)) = (fields.next(), fields.next(), fields.next())
        else {
            continue;
        };
