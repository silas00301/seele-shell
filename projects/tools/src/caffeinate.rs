...
fn command_label(arguments: &[String]) -> String {
    ...
    for value in arguments.iter().skip(1).take(2) {
        if value.is_empty()
            || value.len() > 32
            || value.starts_with('-')
            || !value
...
mod tests {
    ...
    fn a_selected_process_ends_its_session_when_it_exits() {
        ...
        assert_eq!(resolve(&reused).unwrap_err(), "task-unavailable");
        assert_eq!(
            resolve(&format!("process:{}:{started}", child.id())).unwrap_err(),
            "task-unavailable"
        );
    }
    ...
    fn an_unreadable_task_ends_its_session_rather_than_waiting_forever() {
        let state = shared(session("task", None, Some(Tracker::Transfer("gone".into()))));
    ...
    fn selection_keys_reject_anything_they_did_not_issue() {
        ...
        assert!(
            matches!(parse_key("transfer:a:b"), Some(Selector::Transfer(id)) if id == "a:b")
        );
    }
    ...
    fn a_launcher_row_names_the_command_without_its_arguments() {
        ...
        assert_eq!(command_label(&arguments(&["cargo", "-v", "build"])), "cargo");
