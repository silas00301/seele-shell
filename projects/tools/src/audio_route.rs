...
fn owned_name(module: &Value) -> Option<&'static str> {
    if module["name"] != "module-combine-sink" { return None; }
    NAMES.into_iter().find(|name| module["argument"].as_str().unwrap_or("").split_whitespace()
...
fn validate(names: &[String], sinks: &[Value]) -> Result {
    if names.is_empty() { return Err("select at least one output".into()); }
    ...
    for name in names {
        ...
        if name.is_empty() || name.len() > 128
            ...
            || NAMES.contains(&name.as_str()) || !seen.insert(name)
...
pub fn set(names: &[String]) -> Result {
    let runtime = std::env::var_os("XDG_RUNTIME_DIR").ok_or("audio routing needs a user session")?;
    ...
    let modules: Vec<_> = list("modules")?.iter().filter_map(owned_name)
        .filter_map(|name| sinks.iter().find(|sink| sink["name"] == name)
            .and_then(|sink| numeric_id(&sink["owner_module"]))
            .map(|id| (name, id))).collect();
    let previous = output("pactl", ["get-default-sink"]).ok_or("default output unavailable")?;
    let previous = previous.trim();
    let previous_index = sinks.iter().find(|sink| sink["name"] == previous)
    ...
    if names.len() == 1 {
        ...
        target = NAMES.iter().find(|name| !sinks.iter().any(|sink| sink["name"] == **name))
            .ok_or("both Seele output slots are occupied")?.to_string();
        ...
        let id = output("pactl", &args).ok_or("could not combine outputs")?
            .trim().parse::<u64>().map_err(|_| "invalid audio module id")?;
        created = Some(id);
    }
    ...
    let change = (|| -> Result {
        let mut ready = false;
        for _ in 0..50 {
            if list("sinks")?.iter().any(|sink| sink["name"] == target) { ready = true; break; }
            std::thread::sleep(Duration::from_millis(20));
        }
        if !ready { return Err("combined output did not appear".into()); }
        require_status("pactl", ["set-default-sink", target.as_str()])?;
        for stream in &streams {
            // Keep applications explicitly routed elsewhere on their output.
            if previous_index.is_some() && stream["sink"].as_u64() == previous_index
                && !modules.iter().any(|(_, id)| Some(*id) == numeric_id(&stream["owner_module"]))
            {
                let id = stream["index"].as_u64().ok_or("invalid audio stream")?.to_string();
    ...
    if let Err(error) = change {
        ...
        for id in moved { let _ = output("pactl", ["move-sink-input", id.as_str(), previous]); }
        if let Some(id) = created { let _ = output("pactl", ["unload-module", &id.to_string()]); }
        return Err(error);
    }
    ...
    for _ in 0..50 {
        if !list("sinks")?.iter().any(|sink| retired.contains(&sink["name"].as_str().unwrap_or(""))) { return Ok(()); }
        std::thread::sleep(Duration::from_millis(20));
    }
...
mod tests {
    ...
    fn module_arguments_accept_only_available_unique_physical_names() {
        let sinks = vec![json!({"name":"alsa_output.pci-1"}), json!({"name":"bluez_output.AA_1"})];
        assert!(validate(&["alsa_output.pci-1".into(), "bluez_output.AA_1".into()], &sinks).is_ok());
        for names in [vec![], vec!["missing"], vec!["alsa_output.pci-1", "alsa_output.pci-1"],
            vec!["alsa_output.pci-1 sinks=other"], vec!["seele_outputs_a"]] {
            assert!(validate(&names.into_iter().map(String::from).collect::<Vec<_>>(), &sinks).is_err());
        }
    }

    #[test]
    fn cleanup_only_owns_our_combine_modules() {
        assert_eq!(owned_name(&json!({"name":"module-combine-sink","argument":"sink_name=seele_outputs_a sinks=a,b"})), Some("seele_outputs_a"));
        assert!(owned_name(&json!({"name":"module-combine-sink","argument":"sink_name=someone_else sinks=a,b"})).is_none());
        assert!(owned_name(&json!({"name":"module-null-sink","argument":"sink_name=seele_outputs_a"})).is_none());
    }
}
