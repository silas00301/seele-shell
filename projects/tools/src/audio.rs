...
mod tests {
    ...
    fn a_stream_is_named_after_its_program_and_levelled_the_way_wpctl_reports() {
        let entries = streams(&json!([
            stream(
                10,
                json!({"application.name":"Zen Browser","media.name":"A song","application.icon-name":"zen"})
            ),
            stream(11, json!({"node.description":"Bluetooth Receiver"})),
            stream(
                12,
                json!({"application.process.binary":"mpv","media.name":"mpv"})
            ),
            stream(13, json!({})),
        ]));
    ...
    fn system_sounds_and_the_shell_own_output_mirror_never_reach_the_mixer() {
        let entries = streams(&json!([
            ...
            stream(
                31,
                json!({"application.name":"Toast","media.role":"notification"})
            ),
            stream(
                32,
                json!({"node.name":"output.seele_outputs_a_alsa_speakers"})
            ),
            stream(
                33,
                json!({"application.name":"Kept","node.name":"output.other_sink"})
            ),
