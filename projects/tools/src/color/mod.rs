...
fn capture(output: &str, path: &Path, cancel: &AtomicBool) -> Result<Frame> {
    if output.is_empty()
        || output.len() > 256
        || output.starts_with('-')
...
struct Session {
    ...
    samples: Option<mpsc::Sender<Request>>,
    done: JoinHandle<()>,
}
...
fn start(request: Request) -> Session {
    ...
    let (samples, asks) = mpsc::channel::<Request>();
    let done = thread::spawn(move || {
        ...
        let result = (|| -> Result {
            ...
            let mut frames = thread::scope(|scope| {
                let threads: Vec<_> = request
                    ...
                    .map(|(index, output)| {
                        ...
                        let flag = &flag;
                        scope.spawn(move || capture(output, &path, flag))
                    })
                    .collect();
                threads
                    .into_iter()
            ...
            emit(json!({ "id": id, "event": "frames", "captureMs": started.elapsed().as_millis(),
...
mod tests {
    ...
    fn unsafe_output_names_are_rejected_without_capture() {
        for name in ["", "--help", "DP-1\n", "DP-1\0", "DP-1\u{7f}"] {
            assert!(capture(name, Path::new("/unused"), &AtomicBool::new(false)).is_err());
        }
    }
}
