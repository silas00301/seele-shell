...
use seele_runtime::process::{capture_file, discard, stream_stdout_watched, Limits};
...
impl Meter {
    ...
    fn take(&mut self, now: Instant) -> (f32, f32, bool) {
        let peak =
            (f32::from(std::mem::take(&mut self.peak)) / f32::from(i16::MAX)).clamp(0.0, 1.0);
...
/// The report runs for the worker's whole lifetime rather than only while a
/// test does. The panel starts the worker, so the first Record or Start live
/// already knows about an application that took the microphone before the
/// panel was opened, and an idle panel keeps that answer current. A warning
/// does not make the microphone exclusive either, so the report continues
/// while a test runs and another application may take it up afterwards.
...
fn stream_arguments(device: &str) -> Vec<String> {
    vec![
        ...
        "--property=node.dont-reconnect=true".into(),
        "--property=node.dont-fallback=true".into(),
        "--property=node.dont-move=true".into(),
...
/// captured byte; `watch` runs on every supervisor poll and ends the session by
/// cancelling `stop`.
fn capture(
    device: &str,
    stop: &Arc<Stop>,
...
impl Worker {
    ...
    fn start(&mut self, request: &Request) -> std::result::Result<Devices, String> {
        let devices =
            resolve(&request.input, &request.output).map_err(|error| error.to_string())?;
    ...
    fn live(
        ...
        let result = capture(
            ...
            None,
            |chunk| {
                // Linux pipe writes up to PIPE_BUF are atomic. Keep PCM frames
                // aligned even when capture splits a sample or playback stalls.
                ...
                for block in frames.chunks(4096) {
                    match write.write(block) {
                        Ok(count) if count == block.len() => (),
...
pub fn run(arguments: &[String]) -> Result {
    ...
    worker.publish()?;
    // Owned by the process rather than by any one test, so the microphone-use
    // gate is never asked a question it has not yet heard an answer to.
    let watching = Stop::new(&shutdown);
    let watchers = watch_users(watching.clone());
...
mod tests {
    ...
    fn playback_names_one_device_and_never_a_default() {
        ...
        assert!(
            !arguments
                .iter()
                .any(|argument| argument.contains("DEFAULT") || argument.contains("default")),
