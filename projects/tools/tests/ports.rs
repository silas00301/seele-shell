...
impl Host for Recorder {
    ...
    fn signal(&mut self, pid: u32, number: i32) -> bool {
        let mut log = self.0.borrow_mut();
...
fn row<'a>(snapshot: &'a Value, port: u64) -> &'a Value {
    rows(snapshot)
        .iter()
...
fn act(worker: &mut Worker, message: Value) -> (Value, Value) {
    ...
    assert_eq!(replies.len(), 2, "an action reply is followed by a snapshot");
...
fn every_tcp_listener_appears_with_its_actual_binding_and_udp_does_not() {
    ...
    assert_eq!(snapshot["total"], json!(5), "one connection is not a listener");
    ...
    assert_eq!(
        wildcard["scopeLabel"], json!("All interfaces"),
...
fn ownership_is_reported_exactly_and_never_invented() {
    ...
    assert_eq!(
        managed["target"]["privileged"], json!(false),
        "a user service is stopped by the user's own manager"
    );

    let foreign = row(&snapshot, 5432);
    assert_eq!(foreign["user"], json!("root"), "the socket's own uid is known");
    assert_eq!(
...
fn a_shared_socket_requires_a_choice_before_anything_can_be_stopped() {
    ...
    let (reply, snapshot) = act(
        &mut worker,
        json!({"op": "select", "id": id, "pid": 105}),
    );
...
fn a_query_finds_a_listener_by_port_address_process_service_and_project() {
    ...
    let replies = send(&mut worker, json!({"op": "query", "text": "https://localhost:8080"}));
    let snapshot = replies.last().unwrap();
    assert_eq!(snapshot["query"]["scheme"], json!("https"));
    assert_eq!(
        snapshot["query"]["explicitScheme"], json!(true),
        "a typed scheme is preserved for the proposed URL"
    );
    assert_eq!(
        snapshot["total"], json!(5),
...
fn stopping_a_user_service_uses_the_user_manager_and_needs_no_authentication() {
    ...
    log.borrow_mut().properties = "Id=dev.service\nRestart=always\nTriggeredBy=dev.socket\n".to_owned();
    let mut worker = Worker::new(fixture.roots(), &mut recorder, 1000);
    let snapshot = open(&mut worker);
    ...
    assert!(text.contains("Restart=always"), "restart behaviour is disclosed");
    assert!(
    ...
    assert_eq!(log.borrow().units, vec!["--user stop -- dev.service".to_owned()]);
    assert!(
    ...
    assert!(log.borrow().signals.is_empty(), "a service is never stopped by PID");
}
...
fn closing_the_panel_forgets_everything_it_learned() {
    ...
    let snapshot = open(&mut worker);
    assert_eq!(
        row(&snapshot, 5432)["identified"], json!(false),
...
fn refreshing_keeps_row_identity_while_listeners_come_and_go() {
    ...
    assert_eq!(
        row(second, 7000)["id"], json!("4:41007"),
