...
pkgs.stdenvNoCC.mkDerivation {
  ...
  installPhase = ''
    ...
    install -m644 ${./ThemeStore.qml} "$out/share/seele-shell/ThemeStore.qml"
    install -m644 ${./ThemePanel.qml} "$out/share/seele-shell/ThemePanel.qml"
    substituteInPlace "$out/share/seele-shell/ThemePanel.qml" --replace-fail 'import "../shared" as Shared' 'import "shared" as Shared'
    ...
    install -m644 ${./QuickLook.qml} "$out/share/seele-shell/QuickLook.qml"
    ...
    # The theme panel runs the same helper the launcher command runs, found
    # beside the shell's own tools rather than through the session's PATH.
    makeWrapper ${configTools}/bin/seele-theme "$out/bin/seele-theme"
    ...
    makeWrapper ${tools}/bin/seele-color-worker "$out/bin/seele-color-worker" \
      --prefix PATH : "${lib.makeBinPath [ pkgs.grim ]}"
    makeWrapper ${tools}/bin/seele-quicklook "$out/bin/seele-quicklook" \
  ...
  installCheckPhase = ''
    ...
    qmllint -I ${nativeQml}/lib/qt-6/qml -I ${quickshell}/lib/qt-6/qml "$out/share/seele-shell/QuickLook.qml" "$out/share/seele-shell/ResourcesState.qml" "$out/share/seele-shell/ResourcesStore.qml" "$out/share/seele-shell/ResourcesPanel.qml" "$out/share/seele-shell/NetworkActivityStore.qml" "$out/share/seele-shell/NetworkActivityPanel.qml" "$out/share/seele-shell/TextWorkbenchPanel.qml" "$out/share/seele-shell/TextWorkbenchSession.qml" "$out/share/seele-shell/ColorLabPanel.qml" "$out/share/seele-shell/MeetingPlanner.qml" "$out/share/seele-shell/CaffeinateStore.qml" "$out/share/seele-shell/CaffeinatePanel.qml" "$out/share/seele-shell/TransfersStore.qml" "$out/share/seele-shell/TransfersPanel.qml" "$out/share/seele-shell/PortsStore.qml" "$out/share/seele-shell/PortsPanel.qml" "$out/share/seele-shell/ThemeStore.qml" "$out/share/seele-shell/ThemePanel.qml" "$out/share/seele-shell/CalculatorPanel.qml" "$out/share/seele-shell/AiActivityStore.qml" "$out/share/seele-shell/AiActivityPanel.qml" "$out/share/seele-shell/IntegrationHealthStore.qml" "$out/share/seele-shell/SystemHealthPanel.qml" "$out/share/seele-shell/MaintenanceStore.qml" "$out/share/seele-shell/MaintenancePanel.qml" "$out/share/seele-shell/DictationState.qml" "$out/share/seele-shell/shared/"*.qml "$out/share/seele-shell/shell.qml" "$out/share/seele-shell/shared/CenteredGlyph.qml" "$out/share/seele-shell/SystemState.qml" "$out/share/seele-shell/UriPicker.qml" "$out/share/seele-shell/ColorPicker.qml" "$out/share/seele-shell/HeadphonesIcon.qml" "$out/share/seele-shell/NotificationStore.qml" "$out/share/seele-shell/HomeAssistantStore.qml" "$out/share/seele-shell/HomeAssistantPanel.qml" "$out/share/seele-shell/GitHubStore.qml" "$out/share/seele-shell/GitHubInboxStore.qml" "$out/share/seele-shell/GitHubInboxPanel.qml" "$out/share/seele-shell/FocusTimer.qml" "$out/share/seele-shell/FocusPanel.qml" "$out/share/seele-shell/AiPrompt.qml" "$out/share/seele-shell/MicTestStore.qml" "$out/share/seele-shell/MicTestCard.qml"
    ...
    node ${tests}/themes.js "$out/share/seele-shell/ThemeStore.qml" "$out/share/seele-shell/ThemePanel.qml" "$out/share/seele-shell/shell.qml"
    node ${tests}/themes-panel.js "$out/share/seele-shell" ${pkgs.qt6.qtdeclarative}/lib/qt-6/qml
    ...
    node ${tests}/quicklook.js "$out/share/seele-shell/quicklook.js" "$out/share/seele-shell/QuickLook.qml" "$out/share/seele-shell/QuickLookMedia.qml"
    bash ${tests}/quicklook.sh ${tools}/bin/seele-quicklook
