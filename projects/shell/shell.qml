...
Shared.Theme {
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      if (exitCode === 0) {
        ...
        if (root.pendingControlAction === "stream-volume" && root.streamDragId === root.pendingControlValue) {
          root.streamDragId = ""
          root.streamDragValue = -1
        }
      }
  ...
  component ApplicationLevelRow: Row {
    ...
    Rectangle {
      ...
      height: root.rowHeight
      radius: root.radius
      color: applicationMuteMouse.pressed ? root.pressColor : applicationLevelRow.muted ? root.dangerColor : applicationMuteMouse.containsMouse ? root.hoveredColor(root.cardColor) : root.cardColor
      Behavior on color { ColorAnimation { duration: root.durationFast } }
