...
function createStore(publish, arrived, now) {
  var state = { current: [], history: [], dnd: false, dndUntil: 0, dndMinutes: 0, paused: false, lastTick: now, restored: {} }
  state.save = function() {
    var metadata = {}
    ...
    return { history: state.history, dnd: state.dnd, dndUntil: state.dndUntil, dndMinutes: state.dndMinutes, metadata: metadata }
  }
  state.restore = function(saved) {
    if (!saved) return
    state.history = saved.history || []
    state.dnd = !!saved.dnd
    // A quiet period that ran out while the shell was down is over, not owed.
    state.dndUntil = state.dnd && Number.isFinite(saved.dndUntil) && saved.dndUntil > 0 ? saved.dndUntil : 0
    state.dndMinutes = state.dndUntil ? Number(saved.dndMinutes) || 0 : 0
  ...
  state.view = function() {
    ...
    return { count: items.length, items: items, popups: popups, history: state.history, dndUntil: state.dndUntil, dndMinutes: state.dndMinutes }
  }
  ...
  // A quiet period the panel can report an end for. `dndMinutes` records which
  // preset asked for it, so the control that started it is the one shown lit
  // rather than the panel having to guess from a deadline that keeps moving.
