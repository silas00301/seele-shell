...
Scope {
  ...
  // Releases the frozen images and the keyboard. The reading, the chosen
  // format and the history deliberately survive it: the result card is still
  // being read after the overlay has gone, and the next invocation should open
  // in the format the last one ended in.
  function close() {
    if (!active) return
    active = false
    presented = false
  ...
  // near one.
  function present(described) {
    entry = described
    var resolved = Colors.payload(described, picker.format)
