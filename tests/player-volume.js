...
const mutedFirst = {dbusName: 'a', canControl: true, volumeSupported: true, volume: .3};
const mutedSecond = {dbusName: 'b', canControl: true, volumeSupported: true, volume: .8};
level.player = mutedFirst;
handlers.silence();
assert.equal(mutedFirst.volume, 0);
handlers.silence();
assert.equal(mutedFirst.volume, .3, 'a player comes back to the level it was silenced from');
handlers.silence();
level.player = mutedSecond;
handlers.silence();
assert.equal(mutedSecond.volume, 0);
level.player = mutedFirst;
handlers.silence();
assert.equal(mutedFirst.volume, 1, 'the level taken from one player is never handed to another');
assert.equal(mutedSecond.volume, 0, 'restoring one player leaves the other silent');
