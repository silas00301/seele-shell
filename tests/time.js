...
assert(time.filterZones(zones, "london uk").length === 1, "search words may appear in different metadata fields");
assert(time.filterZones(zones, "Europe London").length === 1, "IANA separators are interchangeable with spaces");
assert(time.offsetTime(instant, "+2561", true) === "", "impossible offsets must be rejected");
assert(time.formatOffset("+0545") === "UTC+05:45", "fractional-hour offsets stay readable");
// In a southern-hemisphere zone January is in DST and winter is not. Local
// millisecond arithmetic used to label this Monday as the following week.
assert(time.isoWeek(new Date(2026, 5, 1, 12)) === 23, "ISO weeks must ignore DST");
