...
// Timed quiet mode shares the native store and never replays suppressed toasts.
{
  const h = harness();
  assert.equal(h.store.snooze(15, 1000), true);
  assert.equal(h.dnd, true);
  assert.equal(h.view.dndUntil, 1900);
  assert.equal(h.view.dndMinutes, 15, 'the panel can light the preset that asked for the quiet');
  h.store.receive(h.make(30), 1001);
  assert.equal(h.view.popups.length, 0);
  h.store.advance(1899);
  assert.equal(h.dnd, true);
  h.store.advance(1900);
  assert.equal(h.dnd, false);
  assert.equal(h.view.dndUntil, 0);
  assert.equal(h.view.dndMinutes, 0);
  assert.equal(h.view.items.length, 1);
  assert.equal(h.view.popups.length, 0, 'expiry does not replay the quiet backlog');
  h.store.receive(h.make(31), 1901);
  assert.equal(h.view.popups.length, 1, 'new notifications resume normally');
  h.store.snooze(60, 2000);
  h.store.setDnd(true);
  assert.equal(h.view.dndUntil, 0, 'manual indefinite DND cancels the timer');
  assert.equal(h.view.dndMinutes, 0, 'no preset stays lit once the switch is thrown by hand');
  h.store.advance(9999);
  assert.equal(h.dnd, true);
  h.store.snooze(60, 10000);
  h.store.setDnd(false);
  assert.equal(h.view.dndUntil, 0);
  assert.equal(h.dnd, false);
}
{
  const h = harness();
  for (const duration of [0, -1, 0.5, 1441, NaN, Infinity, 'oops'])
    assert.equal(h.store.snooze(duration, 1000), false);
  assert.equal(h.store.snooze(15, NaN), false);
  assert.equal(h.dnd, undefined, 'invalid requests publish no change');
  h.store.snooze(15, 1000);
  const saved = JSON.parse(JSON.stringify(h.store.save()));
  const restored = harness();
  restored.store.restore(saved);
  assert.equal(restored.dnd, true);
  assert.equal(restored.view.dndUntil, 1900);
  assert.equal(restored.view.dndMinutes, 15);
  restored.store.advance(5000);
  assert.equal(restored.dnd, false, 'suspend/reload retains the absolute deadline');
  const expired = notifications.createStore(() => {}, () => {}, 5000);
  expired.restore(saved);
  assert.equal(expired.dnd, false, 'an expired restored timer cannot suppress a new toast');
  assert.equal(expired.dndUntil, 0);
  expired.restore({dnd:true});
  assert.equal(expired.dnd, true, 'older retained state keeps indefinite DND');
}
console.log('timed Do Not Disturb lifecycle checks passed');

// The preset only fires on a deliberate press: held keys and modified
// shortcuts belong to whatever else is listening.
{
  ...
  quietContext.press({key:3,isAutoRepeat:false,modifiers:1});
  assert.equal(quietStarts, 1, 'quiet presets accept one intentional activation, including numpad Enter');
  console.log('timed DND keyboard guards passed');
}
