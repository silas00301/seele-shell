...
assert.deepEqual(Array.from(state.groups(),group=>group.heading), ['Favorites','Office']);
assert.equal(state.groups()[1].readings[0].state_label,'21');
...
assert.deepEqual(Array.from(state.groups(),group=>group.heading), ['Favorites','Office'], 'favorite readings have a named place in Favorites');
assert.equal(state.groups()[0].readings[0].state_label,'Unavailable');
assert.equal(state.groups()[1].readings[0].state_label,'50');
...
state.preferences = state.entities.map(item=>({entity_id:item.entity_id}));
assert.equal(state.moveTarget('light.desk',1),2,'controls order independently of the readout grid');
state.stopped();
...
console.log('Home Assistant concurrent controls, stale state, credential lifetime and preferences passed');
...
const entries=[{key:'room:Office',heading:'Office'},entity,other];panelMethods.reconcile(rows,entries);const kept=rows.get(1);
...
console.log('Home Assistant shared group/entity model reconciliation passed');
