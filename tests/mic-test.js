...
const store = {
  ...
  users: [], detection: false, usageReady: false, confirming: '', notice: '',
};
...
// Opening the panel must not race the first microphone-use probe.
for (const action of ['sample', 'live']) store.begin(action);
assert.deepEqual(sent, [], 'capture started before the first use probe');
assert.match(store.notice, /Checking microphone use/);
store.accept(JSON.stringify({users: [], detection: true}));
assert.equal(store.usageReady, true);
...
// A restart after a helper crash must not replace that binding with a value:
// the worker would then survive the panel it belongs to.
assert.doesNotMatch(storeSource, /worker\.running\s*=/, 'assigning running would drop the panelOpen dependency');
// The microphone-use report is owned by the worker rather than by a running
// test, so an application holding the microphone before the panel opened still
// gates the first test. That ownership lives in the worker's run loop; this
// fixture only guarantees the store never starts a test without consulting it.
