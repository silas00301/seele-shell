...
state.format = "rgb"
state.open()
assert.equal(state.format, "hex", "each picker invocation starts with the no-choice hex default")
state.close()
...
// A pointer pick is always the hex fast path, independently of the keyboard's
// current format choice.
state.format = "rgb"
state.commit("hex")
assert.equal(state.commitFormat, "hex")
state.accept({id: state.generation, event: "sample", token: state.asked, commit: true, output: "DP-1", r: 0x12, g: 0x34, b: 0x56})
assert.equal(state.clipboard.payload, "#123456")
